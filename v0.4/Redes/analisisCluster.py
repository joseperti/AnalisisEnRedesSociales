    ...
    def popup(self,event):
        ...
        for k in self.nodos:
            ...
            if (actual.colision(x, y)):
                ...
                self.menuNodo.add_command(label="Información de nodo", command=lambda: self.mostrarInfoExplNodo(actual))
                self.menuNodo.post(event.x_root, event.y_root)
                break

    def mostrarInfoExplNodo(self,nodo):
        nodo.ventanaInfoNodo()
    ...
    def pintarCluster(self):
        ...
        for k in self.nodos:
            ...
            for l in self.nodos[k].getDestinos():
                ...
                self.canvas.create_line(posIni[0], posIni[1], posDest[0], posDest[1], fill=tcolor)
            if self.nodos[k].getMostrarInfo():
                tcolor = "light goldenrod"
                for l in self.nodos[k].getRelacionesIn():
                    posDest = self.nodos[l].getPos()
                    self.canvas.create_line(posIni[0], posIni[1], posDest[0], posDest[1], fill=tcolor)
                tcolor = "medium orchid"
                for l in self.nodos[k].getRelacionesOut():
                    posDest = self.nodos[l].getPos()
                    self.canvas.create_line(posIni[0], posIni[1], posDest[0], posDest[1], fill=tcolor)
