...
class VentanaRed:
    ...
    def addRelacion(self,identificador1,identificador2):
        ...
        self.nodos[identificador1].addEdge(identificador2,None)
        self.nodos[identificador1].addRelacionOut(identificador2)
        #Bidireccional
        self.nodos[identificador2].addEdge(identificador1,None)
        self.nodos[identificador2].addRelacionIn(identificador1)

    def addRelacionPeso(self, identificador1, identificador2, peso):
        self.addNodo(identificador1)
        self.addNodo(identificador2)
        self.nodos[identificador1].addEdge(identificador2,peso)
        self.nodos[identificador1].addRelacionOut(identificador2)
        # Bidireccional
        self.nodos[identificador2].addEdge(identificador1,peso)
        self.nodos[identificador2].addRelacionIn(identificador1)
        #print("Añadida relación de: "+str(identificador1)+" a: "+str(identificador2)+" peso: "+str(peso))
    ...
    def pintarRed(self):
        ...
        for k in self.nodos:
            self.pintarNodoRels(k)

    def pintarNodoRels(self,k):
        posIni = self.nodos[k].getPos()
        for l in self.nodos[k].getDestinos():
            posDest = self.nodos[l].getPos()
            peso = self.nodos[k].getPeso(l)
            if peso is None:
                self.canvas.create_line(posIni[0], posIni[1], posDest[0], posDest[1])
            elif peso == 'negative':
                self.canvas.create_line(posIni[0], posIni[1], posDest[0], posDest[1],fill="red")
            if peso == 'positive':
                self.canvas.create_line(posIni[0], posIni[1], posDest[0], posDest[1], fill="green")
        if self.coloreado == "1":
    ...
    def exportarCSV(self):
        nombre = asksaveasfile(mode='w', defaultextension='.csv')
        nombre.close()
        with codecs.open(nombre, mode='w',encoding='utf-8') as archivo:
            if archivo is None:
                return
    ...
    def setTextoInfoNodo(self,nodo,texto):
        self.nodos[nodo].setTextoInfo(texto)
