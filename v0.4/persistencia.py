...
class Persistencia():
    ...
    def getTweetFecha(self,identificador,year=None,month=None,day=None,hour=None,minute=None):
        variable = dict()
        ...
        resultado = self.dbTweet.find(variable)
    ...
    def getDateSeguimiento(self,identificador,opcion = "Minuto"):
        ...
        if opcion == "Minuto":
            resultado = self.dbTweet.aggregate(
               ...
               { "$group": {"_id":"$fecha","fecha":{"$first":"$fecha"}, "count":{"$sum":1 }} },
               { "$sort": { "_id":1 } }
               ]
            )
            ...
            resultado = self.dbTweet.aggregate(
                [
                    {"$match": {"identificador": identificador}},
                    {"$group": {"_id": {"Año": "$year", "Mes": "$month", "Día": "$day", "Hora": "$hour","Minuto":"00"},"fecha":{"$first":"$fecha"}, "count": {"$sum": 1}}},
                    {"$sort": {"_id": 1}}
                ]
            )
        elif opcion == "Día":
            resultado = self.dbTweet.aggregate(
                [
                    {"$match": {"identificador": identificador}},
                    {"$group": {"_id": {"Año": "$year", "Mes": "$month", "Día": "$day","Hora": "00","Minuto":"00"},"fecha":{"$first":"$fecha"}, "count": {"$sum": 1}}},
                    {"$sort": {"_id": 1}}
                ]
            )
        elif opcion == "Mes":
            resultado = self.dbTweet.aggregate(
                [
                    {"$match": {"identificador": identificador}},
                    {"$group": {"_id": {"year": "$year", "month": "$month", "Día": "1","Hora": "00","Minuto":"00"},"fecha":{"$first":"$fecha"}, "count": {"$sum": 1}}},
                    {"$sort": {"_id": 1}}
                ]
            )
        lista = []
