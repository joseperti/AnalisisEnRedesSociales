...
class Interfaz:
    ...
    def cargarPanel(self):
        ...
        self.listaStreams = Listbox(self.root,selectmod=EXTENDED)
        ...
        self.menuListaStreams.add_command(label = "Estudio de relación Seguimientos",command=self.estudioRelacionSeguimientos)
    ...
    def estudioRelacionSeguimientos(self):
        ...
        for k in identificadores:
            for j in identificadores:
                if j!=k:
                    print("De "+k+" en "+j+" : "+str(persistencia.enlacesIdentificadores(j,k)))
